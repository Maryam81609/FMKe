...
init(Req0, Opts) ->
  try
    ...
    Req = handle_req(Method, HasBody, Opts, Req0),
    {ok, Req, Opts}
  catch
    Err:Reason ->
...
handle_req(<<"POST">>, true, _Opts, Req) ->
  create_pharmacy(Req);
handle_req(<<"POST">>, false, _Opts, Req) ->
  cowboy_req:reply(400, #{}, ?ERR_MISSING_BODY, Req);

handle_req(<<"PUT">>, true, _Opts, Req) ->
  update_pharmacy(Req);
handle_req(<<"PUT">>, false, _Opts, Req) ->
  cowboy_req:reply(400, #{}, ?ERR_MISSING_BODY, Req);

handle_req(<<"GET">>, true, _Opts, Req) ->
  cowboy_req:reply(400, #{}, ?ERR_BODY_IN_A_GET_REQUEST, Req);
handle_req(<<"GET">>, false, prescriptions, Req) ->
  get_pharmacy_prescriptions(Req);
handle_req(<<"GET">>, false, processed_prescriptions, Req) ->
  get_pharmacy_processed_prescriptions(Req);
handle_req(<<"GET">>, false, _Opts, Req) ->
  get_pharmacy(Req).
...
get_pharmacy_processed_prescriptions(Req) ->
  ...
  case IntegerId =< ?MIN_ID of
    true ->
      cowboy_req:reply(400, #{}, ?ERR_INVALID_PHARMACY_ID, Req);
    false ->
      ServerResponse = fmk_core:get_processed_pharmacy_prescriptions(IntegerId),
      JsonReply = jsx:encode([{success, true}, {result, ServerResponse}]),
      cowboy_req:reply(200, #{
        <<"content-type">> => <<"application/json">>
      }, JsonReply, Req)
