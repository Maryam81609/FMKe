...
init(Req0, Opts) ->
  try
    ...
    Req = handle_req(Method, HasBody, Req0, Opts),
    {ok, Req, Opts}
  catch
    Err:Reason ->
...
handle_req(<<"POST">>, true, Req, _Opts) ->
  create_staff(Req);
handle_req(<<"POST">>, false, Req, _Opts) ->
  cowboy_req:reply(400, #{}, ?ERR_MISSING_BODY, Req);

handle_req(<<"PUT">>, true, Req, _Opts) ->
  update_staff(Req);
handle_req(<<"PUT">>, false, Req, _Opts) ->
  cowboy_req:reply(400, #{}, ?ERR_MISSING_BODY, Req);

handle_req(<<"GET">>, true, Req, _Opts) ->
  cowboy_req:reply(400, #{}, ?ERR_BODY_IN_A_GET_REQUEST, Req);
handle_req(<<"GET">>, false, Req, prescriptions) ->
  get_prescriptions(Req);
handle_req(<<"GET">>, false, Req, _Opts) ->
  get_staff(Req).
  ...
  case IntegerId =< ?MIN_ID of
    ...
    false ->
      ...
      JsonReply = case Success of
                    ...
                    false ->
                      jsx:encode([{success, Success}, {result, <<"not found">>}])
                  end,
      cowboy_req:reply(200, #{
        <<"content-type">> => <<"application/json">>
      }, JsonReply, Req)
...
get_prescriptions(Req) ->
  ...
  case IntegerId =< ?MIN_ID of
    ...
    false ->
      ServerResponse = fmk_core:get_staff_prescriptions(IntegerId),
